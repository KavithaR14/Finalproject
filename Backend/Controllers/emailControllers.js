...
const generatePDF = async (userName, chartImage, sectionScores, sectionDescriptions) => {
  ...
  if (chartImage) {
    ...
    // Add top spacing and keep left position
    const topSpacing = -10; // Increase this value for more top space
    const chartX = -20;
    const chartY = (height - chartHeight) / 2 + topSpacing; // Added top spacing
